    ...
    if selected_doc_type == "SO":
        with col_kiri:
            with st.container(border=True):
                ...
                # =====================================================
                with st.expander("🧪 Debug Total SO — PostgreSQL", expanded=False):
                    st.caption(
                    ...
                    with r1:
                        metric_card(
                            "Header Gross SO (Control)",
                            f"Rp {total_so_header_gross:,.0f}".replace(",", "."),
                        )
                    with r2:
                        metric_card(
                            "Excluded Keyword (Header Control)",
                            f"Rp {total_so_excluded_keyword:,.0f}".replace(",", "."),
                        )

                    st.caption(
                        "Card Total SO memakai SUM current SO detail setelah status dan keyword filter. "
                        "Header transaction_total hanya ditampilkan sebagai control/audit. "
                        "Tax hasil derivasi yang sangat dekat dengan integer dinormalisasi "
                        "(contoh 10.999999 → 11) untuk menjaga parity dengan API."
                    )
                    ...
                    mapping_cols = [c for c in [
                        "reader_soft_delete_rule",
                        "reader_line_transaction_total_col",
                        "reader_pic_source", "reader_pic_master",
                        "reader_product_source", "reader_product_master",
                    ] if c in df_so_final.columns]
                    if mapping_cols and not df_so_final.empty:
                        with st.expander("🔎 Mapping Reader PostgreSQL", expanded=False):
                            mapping_preview = (
                                df_so_final[mapping_cols]
                                .drop_duplicates()
                                .head(20)
                            )
                            st.dataframe(
                    ...
                    st.download_button(
                        label="⬇️ Download Debug Total SO — PostgreSQL.xlsx",
                        data=debug_total_so_excel,
                        file_name=(
                            "Debug_Total_SO_postgresql_"
                        ...
                        key="download_debug_total_so_postgresql",
                        use_container_width=True,
                    )
                    st.caption(
                ...
                # =====================================================
                # PHASE 5 DEBUG: REVENUE / SO BALANCE EVIDENCE / PIC SALES
                # =====================================================
                with st.expander("🧪 Debug Revenue — PostgreSQL", expanded=False):
                    st.caption(
                        "Revenue metric sekarang memakai SI dengan transaction_date di dalam "
                        "Select Date Range. Dataset SI cumulative tetap dipakai terpisah untuk chain transaksi."
                    )
                    ...
                    st.download_button(
                        "⬇️ Download Debug Revenue — PostgreSQL.xlsx",
                        data=debug_revenue_excel,
                        file_name="Debug_Revenue_postgresql_" + datetime.now().strftime("%Y%m%d_%H%M%S") + ".xlsx",
                        mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
                        key="download_debug_revenue_pg",
                        use_container_width=True,
                    )

                with st.expander("🧪 Debug SO Balance — PostgreSQL", expanded=False):
                    st.caption(
                        "SO Balance FINAL: hanya SO yang belum memiliki DO valid (NO DO) atau "
                        "DO-nya masih parsial (PARTIAL DO). PR dan SI tidak mengurangi balance."
                    )
                    ...
                    if debug_so_balance_status is not None and not debug_so_balance_status.empty:
                        ...
                        st.markdown("**Breakdown Status SO Balance**")
                        st.dataframe(status_bal_display, use_container_width=True, hide_index=True)
                    st.download_button(
                        "⬇️ Download Debug SO Balance — PostgreSQL.xlsx",
                        data=debug_so_balance_excel,
                        file_name="Debug_SO_Balance_postgresql_" + datetime.now().strftime("%Y%m%d_%H%M%S") + ".xlsx",
                        mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
                        key="download_debug_balance_pg",
                        use_container_width=True,
                    )

                with st.expander("🧪 Debug PIC Sales — PostgreSQL", expanded=False):
                    pic_nonblank = 0
                    pic_unique = 0
                    ...
                    if not pic_candidate_cols.empty:
                        st.markdown("**Kandidat kolom PIC/Sales/User pada tabel SO**")
                        st.dataframe(pic_candidate_cols, use_container_width=True, hide_index=True)
                    if not pic_foreign_keys.empty:
                        st.markdown("**Foreign Key x4_sales_order / detail**")
                        st.dataframe(pic_foreign_keys, use_container_width=True, hide_index=True)
                    if not pic_header_sample.empty:
                        st.markdown("**Sample nilai kandidat dari header SO terbaru**")
                        st.dataframe(pic_header_sample, use_container_width=True, hide_index=True)
                    st.download_button(
                        "⬇️ Download Debug PIC Sales — PostgreSQL.xlsx",
                        data=debug_pic_excel,
                        file_name="Debug_PIC_Sales_postgresql_" + datetime.now().strftime("%Y%m%d_%H%M%S") + ".xlsx",
                        mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
                        key="download_debug_pic_pg",
                        use_container_width=True,
                    )
